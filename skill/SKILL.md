...
9. Run `proof` with representative queries to capture quality, top hits, and context-reduction evidence.
...
16. Run `pack` for graph hits that would otherwise become large raw context; use live/graph/offloaded buckets before handing context to an agent.
